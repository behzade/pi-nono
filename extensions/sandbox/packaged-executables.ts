...
export function resolvePackagedExecutables(
	...
	if (!selected) {
		throw new Error(`pi-nono npm packages do not support ${platform}/${arch}`);
	}
	...
	try {
		...
		throw new Error(
			`pi-nono native package ${selected.name}@${NATIVE_VERSION} is missing`,
			{ cause: error },
		);
	}
	...
	if (manifest.name !== selected.name || manifest.version !== NATIVE_VERSION) {
		throw new Error(
			`pi-nono native package must be ${selected.name}@${NATIVE_VERSION}; found ${stringField(manifest.name)}@${stringField(manifest.version)}`,
		);
	}
	if (
		!arrayField(manifest.os).includes(platform) ||
		!arrayField(manifest.cpu).includes(arch) ||
		manifest.piNono?.nono?.path !== selected.nonoPath
	) {
		throw new Error(`pi-nono native package ${selected.name} has invalid platform or executable metadata`);
	}
	const root = dirname(manifestPath);
	return {
		nonoPath: verifiedExecutable(root, selected.nonoPath, manifest.piNono?.nono?.sha256),
...
function parseManifest(path: string): NativeManifest {
	try {
		...
		throw new Error(`pi-nono native package manifest is invalid: ${path}`, { cause: error });
	}
}
...
function verifiedExecutable(root: string, relativePath: string, expectedSha256: unknown): string {
	if (typeof expectedSha256 !== "string" || !/^[a-f0-9]{64}$/.test(expectedSha256)) {
		throw new Error(`pi-nono native executable ${relativePath} has invalid checksum metadata`);
	}
	...
	if (!metadata.isFile() || metadata.isSymbolicLink() || (metadata.mode & 0o111) === 0) {
		throw new Error(`pi-nono native executable must be a regular executable file: ${lexicalPath}`);
	}
	const path = realpathSync(lexicalPath);
	const fromRoot = relative(root, path);
	if (fromRoot === ".." || fromRoot.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`) || isAbsolute(fromRoot)) {
		throw new Error(`pi-nono native executable escapes its package: ${lexicalPath}`);
	}
	const actualSha256 = createHash("sha256").update(readFileSync(path)).digest("hex");
	if (actualSha256 !== expectedSha256) {
		throw new Error(`pi-nono native executable checksum mismatch: ${lexicalPath}`);
	}
	return path;
}
